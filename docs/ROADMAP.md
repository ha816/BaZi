...
- [ ] `SummaryCard` 프론트
- [ ] AI 풀이·챗 인트로 정합
- [ ] `test_interpret.py`에 summary 필드 테스트
- [ ] §6 관찰 1회
...
- [ ] 도메인 매핑 표 확정 (명리 근거 검토)
- [ ] `upcoming_months` 12개월 확장 + 사용처 회귀 확인
- [ ] `_get_month_domain_scores` + `timing` 필드
- [ ] `TimingTab`
- [ ] 챗 컨텍스트 주입
- [ ] 테스트
...
|---|---|---|---|---|---|---|
| F0 | 측정 기반 (이벤트 로깅) | 🟢 | ⬜ 대기 | | | R1과 같은 스프린트 |
| R1 | 첫 화면 요약 카드 | 🟢 | ⬜ 대기 | | | 추천 시작점 |
| R2-1 | 출생시간 모름 고지 | 🟢 | ⬜ 대기 | | | |
| R2-2 | 세 기둥 계산 | 🔴 | ⬜ 대기 | | | R2-1 후 |
| R3 | 타이밍 리포트 | 🟡 | ⬜ 대기 | | | 프리미엄 후보 |
| R4 | 아침 알림 + 홈 한 문장 | 🟡 | ⬜ 대기 | | | 소유자 검증 선행 |
| R5 | 궁합 공유 + 오늘의 궁합 | 🟡 | ⬜ 대기 | | | |
