...
def test_element_balance_section():
    ...
    assert any("기운이 강한" in line or "기운이 약한" in line or "균형을 이루고" in line for line in result.element_balance)


def test_yongshin_section():
    result = _make_postnatal_result()
    assert len(result.yongshin) > 0
    assert any("기운" in line for line in result.yongshin)
...
def test_advice_section():
    ...
    assert any("개운법" in line or "색상" in line or "기운" in line for line in result.advice)
