...
from bazi.application.interpret.advice import get_advice
from bazi.application.interpret.daeun import get_major_fortune
from bazi.application.interpret.fortune import get_fortune_by_domain
from bazi.application.interpret.personality import get_element_balance, get_personality
from bazi.application.interpret.relationship import get_relationships
from bazi.application.interpret.seun import get_annual_fortune
from bazi.application.interpret.yongshin import get_yongshin
...
class Interpreter:
    ...
    def __call__(self, natal: NatalInfo, postnatal: PostnatalInfo) -> Interpretation:
        return Interpretation(
            **_build_chart_data(natal, postnatal),
            personality=get_personality(natal),
            element_balance=get_element_balance(natal),
            yongshin=get_yongshin(natal, postnatal),
            fortune_by_domain=get_fortune_by_domain(postnatal),
            annual_fortune=get_annual_fortune(natal, postnatal),
            major_fortune=get_major_fortune(postnatal),
            relationships=get_relationships(postnatal),
            advice=get_advice(natal, postnatal),
        )
