...
class ProfileService:
    def __init__(
        ...
        fortune_port: FortunePort | None = None,
    ):
    ...
    async def update_profile(
        ...
        updated = await self.profile_port.update(profile_id, name, gender, birth_dt, city, birth_hour_unknown=birth_hour_unknown)
        # 출생정보(시각·시간 미상 포함)가 바뀌면 연도·일진 캐시는 모두 무효
        await self.analysis_port.delete_by_profile(profile_id)
