...
class LlmReportBuilder:
    ...
    def _intro(self) -> str:
        return (
            "# 사주 분석 레포트\n\n"
            "아래 데이터로 자연스러운 한국어 통합 해석을 작성해주세요.\n\n"
            "**문체**: 각 섹션은 `👉 \"…\"` 굵은 결론 먼저, 근거 괄호 병기(예: `(정관 강함)`). "
            "짧은 문장+불릿, 구어체 존댓말. 전문용어는 우리말 먼저+한자 괄호 병기.\n\n"
            "**포맷 규칙(엄수)**: 마크다운 표(`|...|`) 절대 사용 금지. "
            "월별 가이드 등 나열은 반드시 한 줄에 하나씩 불릿으로 작성하세요. "
            "예: `- 5월 癸巳 (용신·충): 새 시도 좋음`. 각 불릿은 줄바꿈으로 명확히 구분.\n\n"
            "**섹션**: ①본질·자아 ②에너지(오행·십신 결핍) ③신살(있으면) ④올해 전략(세운+대운+충합) "
            "⑤재물·성취 ⑥월별 가이드(다가올 6개월 핵심만) ⑦건강·심리 ⑧풍수·개운법. "
            "마지막에 메타포 한 줄 + 오늘 할 수 있는 행동 하나로 마무리."
        )
